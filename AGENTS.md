...
- **Storage bindings** (see `wrangler.toml.example` for the committed template; the actual `wrangler.toml` is gitignored and generated by `npm run setup`): `MY_BUCKET` (R2), `DB` (D1). Vars: `R2_ACCOUNT_ID`, `R2_BUCKET_NAME`. Secrets: `API_KEY`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `ALLOWED_ORIGINS`.
...
wrangler.toml.example      # Committed Worker config template (empty account_id / database_id).
wrangler.toml              # Local Worker config (gitignored, generated by `npm run setup`).
